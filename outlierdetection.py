...
"""
def get_featureindex(feature, list='featuremap.featuremap'):
...
def clean_from_metaindex(metadata, dataset, index, minval=None, maxval=None, equal=None):
	c=0
...
"""
This function removes all datapoints with more than one author
"""
def max_one_author(metadata, dataset):
	index_of_authorcount = get_featureindex('number of authors', 'featuremap.metadatamap')
	maxoneauthor_meta, maxoneauthordata, num_removed = clean_from_metaindex(meta, dataset, index_of_authorcount, maxval=1)
	return maxoneauthor_meta, maxoneauthordata, num_removed
...
print get_featureindex('WB_Yule')
